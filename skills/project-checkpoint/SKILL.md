...
description: Create, update, compact, or export a project checkpoint for ongoing work. Use for checkpoint, continue, fresh chat, handoff, resume, major approval, milestone, or context-heavy project turns.
---
...
Use this skill when the user says "checkpoint", "continue", "fresh chat", "handoff", "resume", or "what's next"; when a project has multiple files, artifacts, prior chats, or settled decisions; when the user approves a major decision; when context is getting heavy; when a milestone is completed; or when the user needs a paste-ready continuation prompt.
...
Approved: [settled decisions]
Working set: [files/artifacts relevant now]
Ignore for now: [stable, out-of-scope, or already-settled items]
Current task: [what is being done now]
Next: [single best continuation]

Fresh-chat prompt:
[compact copy-paste prompt for continuing in a new chat, only if useful]
```

Optional fields: `Blockers`, `Risks`, `Recent changes`, `Validation`, and `Git state`. Include them only when they are useful.
