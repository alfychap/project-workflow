...
description: Recommend the right execution surface, model, reasoning level, tools, plugins, and skills for a task from the capabilities actually available in the active environment. Use when the user asks how to run, route, delegate, tool, model, or staff a task.
---
...
Usage forecasting belongs to Usage Gauge. You may mention obvious cost or context tradeoffs qualitatively, but do not estimate quotas, token totals, or account limits here.
