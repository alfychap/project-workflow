---
name: pw-project-structure
description: Organize a project, repository, or artifact set and create or update a concise PROJECT.md when useful. Use for placement, naming, Git workflow, or archive structure; do not use it for current-state reporting or implementation specs.
---

# 🗂️ PW Project Structure
...
Return the saved brief link, a short account of meaningful updates, and any justified artifact suggestions. `pw-messy-material` owns restructuring supplied material, `pw-project-status` owns current-state assessment, and `pw-to-spec` owns an authoritative implementation spec. Do not run them automatically. `/structure` is a conversational convention where this skill is available, not a guaranteed host command.
